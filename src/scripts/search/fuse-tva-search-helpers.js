...
export async function doImageSearch(
  ...
  debug("STARTING: Art Search", search, searchType, searchOptions);
...
async function _randSearchUtil(
  ...
  const randSettings = randomizerOptions; //mergeObject(randomizerOptions, TVA_CONFIG.randomizer, { overwrite: false });
  if (
    !(
...
//  * @returns void
//  */
// async function walkFindImages(path, { apiKey = "" } = {}, found_images, imgurClientId = null) {
//   let files = {};
//   if (!path.source) {
//     path.source = "data";
//   }
...
//   } catch (err) {
//     warn(
//       `${game.i18n.localize("token-variants.notifications.warn.path-not-found")} ${path.source}:${path.text}`
//     );
//     return;
//   }
...
export async function findImagesFuzzy(name, searchType, searchOptions, found_images = [], forceSearchName = false) {
  log("STARTING: Fuzzy Image Search", name, searchType, searchOptions, forceSearchName);
  ...
  debug("ENDING: Fuzzy Image Search", results);

  return results;
}

async function findImagesExact(name, searchType, searchOptions, found_images = []) {
  debug("STARTING: Exact Image Search", name, searchType, searchOptions);
  ...
  debug("ENDING: Exact Image Search", matchedImages);
  return matchedImages;
}

async function findImages(name, searchType = "", searchOptions = {}) {
  const sOptions = searchOptions; // mergeObject(searchOptions, getSearchOptions(), { overwrite: false });
