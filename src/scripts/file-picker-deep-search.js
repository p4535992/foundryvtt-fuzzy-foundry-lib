...
export class FilePickerDeepSearch {
  ...
  async buildAllCache(force = false) {
    ...
    if (!localCache) storedCacheResponse = await fetch(jsonPath ?? prefixURL + "/DigDownCache.json");
  ...
  async buildCache(dir, type = "user") {
    ...
    for (let path of content.files) {
      ...
      if (
        ...
        (this._excludeKeywords?.length && this._excludeKeywords.some((k) => path.includes(k)))
      )
        continue;
      const fileName = path.split("/").pop();
  ...
  static async _onSearchFilter(wrapped, event, query, rgx, html) {
    ...
    let folder = $(this.element).find(`input[name="target"]`)[0].value.replaceAll(" ", "%20");
    if (folder !== "") {
